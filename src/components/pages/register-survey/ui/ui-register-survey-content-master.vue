...
<fragment>
	<b-row>
    <b-col>
			<button @click="gotoPage('/register-survey-tambah')" class="btn btn-add">+ Tambah Data Survey</button>
		</b-col>
    <b-col>
  ...
  name: 'ui-penerima-pemohon-content-master',
	...
	methods: {
		initDatabase(){
			...
			if(this.getters_survey) {
				// Request from vuex
				this.database = this.getters_survey
			}
		},
		onEdit(data) {
			// console.log(data)
			this.$router.push({
					...
					params: {
						id: data.index //data.item.kode_survey
					}
			 })
		},
		onDelete(index){
				...
				vm.$axios.get(ipAPI)
				  .then(function (response) {
				    ...
				    console.log(response);
						vm.dispatch_survey_delete_payload_items({ index })
				  })
