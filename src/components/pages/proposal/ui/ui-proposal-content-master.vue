...
<fragment>
	...
	<div class="mt-3 p-4 bg-white">
	  ...
	  <b-table id="master" bordered show-empty responsive hover sticky-header v-if="mounted" :items="database.payload.payload_items" :fields="database.reference.reference_fields">
			...
			<template v-slot:head(nomor_proposal)="data">
				<div class="table-header p-2" style="width:100px">
					<span class="">{{ data.label }}</span>
				</div>
			</template>
			<template v-slot:head(status_proposal)="data">
				<div class="table-header p-2" style="width:150px">
					<span class="">{{ data.label }}</span>
				</div>
			</template>
...
export default{
	...
	methods: {
		initDatabase(){
			...
			if(this.getters_proposal) {
				// Request from vuex
				this.database = this.getters_proposal
			}
		},
		onEdit(data) {
			// console.log(data)
			this.$router.push({
					...
					params: {
						id: data.index //data.item.kode_proposal
					}
			 })
		},
		onDelete(index){
				...
				vm.$axios.get(ipAPI)
				  .then(function (response) {
				    ...
				    console.log(response);
						vm.dispatch_proposal_delete_payload_items({ index })
				  })
