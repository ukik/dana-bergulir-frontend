    ...
    >
      ...
      <div class="m-0 p-4 section-parent">
        ...
        >
          ...
          <div>
            <span class="subtitle">DETAIL AGUNAN</span>
          </div>
          ...
          <b-table-simple class="table b-table table-hover bg-white table-bordered">
            <b-tbody>
              <b-tr v-for="(item, index) in parent_item.payload_items_detail_angunan" :key="index">
                <b-td colspan="1" >{{ item.deskripsi }}</b-td>
                <b-td colspan="1" width="140">
                  <input
                    :name="parent_index+'payload_items_detail_angunan'+index"
                    :id="parent_index+'payload_items_detail_angunan'+index"
                    :required="true"
                    v-model="item.value"
                    :type="item.type"
                    min="0"
                  ...
                  <!-- Required -->
                  <p
                    v-if="fieldErrors[parent_index+'payload_items_detail_angunan'+index]"
...
export default {
  watch: {
    database: {
      handler(val) {
        ...
        this.dispatch_analisa_proposal_tahap_3(val);
      },
      deep: true,
      immediate: false
    }
  ...
  created() {
    ...
    if (this.getters_analisa_proposal_tahap_3) {
      // Request from vuex
      this.database = this.getters_analisa_proposal_tahap_3;
    }
  ...
  methods: {
    ...
    onSubmit: window._.debounce(function() {
      ...
      new Promise(resolve => {
        resolve(
          this.dispatch_analisa_proposal_tahap_3_edit_payload_items({
