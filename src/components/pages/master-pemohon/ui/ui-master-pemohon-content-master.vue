...
<fragment>
	<b-row>
    <b-col>
			<button @click="gotoPage('/master-pemohon-tambah')" class="btn btn-add">+ Tambah Data UKM Baru</button>
		</b-col>
    <b-col>
	...
	<div class="mt-3 p-4 bg-white">
	  ...
	  <b-table id="master" bordered show-empty responsive hover sticky-header v-if="mounted" :items="database.payload.payload_items" :fields="database.reference.reference_fields">
			<template v-slot:head()="data">
				<div class="table-header p-2" style="width:200px">
					<span class="">{{ data.label }}</span>
				</div>
			</template>
			<template v-slot:head(nomor)="data">
				<div class="table-header p-2" style="width:50px">
					<span class="">{{ data.label }}</span>
				</div>
			</template>
...
import { database } from '@/models/master-pemohon'
  ...
  name: 'ui-penerima-pemohon-content-master',
	...
	methods: {
		initDatabase(){
			...
			if(this.getters_pemohon) {
				// Request from vuex
				this.database = this.getters_pemohon
			}
		},
		onEdit(data) {
				...
				vm.$axios.get(ipAPI)
				  .then(function (response) {
				    ...
				    console.log(response);
						vm.dispatch_pemohon_delete_payload_items({ index })
				  })
