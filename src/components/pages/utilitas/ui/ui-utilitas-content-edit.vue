	        ...
	        <form class="col-8" ref="form" @submit.prevent="onSubmit" @change="handleChange" @invalid.capture.prevent="handleInvalid">
	            ...
	            <b-row class="mb-2">
	                ...
	                <b-col>
	                    ...
	                    <div class="input-group">
	                        :
	                        <b-form-file v-model="database.payload.foto" @change="onImageChange" :required="false" id="foto" name="foto" class="form-control ml-2" placeholder="Pilih foto profile..." drop-placeholder="Drop file here..."></b-form-file>
	                    </div>
	                    <!-- Required -->
...
export default {
	...
	computed: {
		image_placeholder() {
			return !this.database.payload.foto ? "Pilih foto profile..." : this.database.payload.foto.name
		}
	},
	methods: {
		initDatabasePayload(){
			const get = this.getters_utilitas_edit({
		...
		onSubmit: window._.debounce(function() {
			...
			new Promise(resolve => {
				resolve(this.dispatch_utilitas_edit_payload_items({
					index: this.$route.params.id,
